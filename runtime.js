...
await processStyles()
  ...
  console.error("Error processing fscss: ", err);
}
  }
...
async function processElement(el) {
  ...
  let css = await process(text);
  css = inline(css);

  el.setAttribute("style", css);
}

async function applyFscssStyles() {
  const fscssLinks = document.querySelectorAll('[type*="fscss"]');
  
  // Return early if no FSCSS links exist
  if (fscssLinks.length === 0) return;

  // Fetch all link sources concurrently
  const promises = Array.from(fscssLinks).map(async (link) => {
    try {
  ...
  // 1. Wait for all fetches to resolve
  const cssContents = await Promise.all(promises);

  // 2. Inject all fetched CSS into the DOM synchronously
  const fragment = document.createDocumentFragment();
  cssContents.forEach(async (css) => {
    if (css) {
...
async function main() {
  try {
    await applyFscssStyles(); // fetch + inject only
    await processStyles(); // heavy processing – run ONCE
    // Process any inline style attributes
...
function exec({ type = 'text', content, onError, onSuccess }) {
  ...
  // 2. Normalizing Types
  const isText = ['text', 'auto', 'text/fscss', 'text/css', 'text/xfscss', 'fscss'].includes(type);
  const isUrl = ['fromUrl', 'URL', 'fromURL', 'link', 'external'].includes(type);
